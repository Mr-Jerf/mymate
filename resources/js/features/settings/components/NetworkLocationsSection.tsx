...
type StatusPageSettings = { brand_name: string; subtitle: string; poll_ms: number; show_site_names: boolean; show_device_counts: boolean; allow_subscriptions: boolean; public_enabled: boolean; monitoring_grace_minutes: number; logo_url: string; favicon_url: string; color_operational: string; color_degraded: string; color_monitoring: string; color_outage: string; color_unknown: string; color_maintenance_scheduled: string; color_maintenance_active: string; color_maintenance_completed: string };
        ...
        <section className="space-y-6">
            {statusDraft && <section className="min-w-0 rounded-2xl bg-white/[0.02] p-5 ring-1 ring-white/[0.06]">
                ...
                <div className="grid gap-2.5 md:grid-cols-2">
                    ...
                    <label className="flex items-center gap-2 text-xs text-white/70"><span>Monitoring grace (minutes)</span><input className="w-24 rounded-lg bg-white/[0.04] px-3 py-2 text-sm text-white ring-1 ring-white/10" type="number" min={30} max={120} value={statusDraft.monitoring_grace_minutes} onChange={(e) => setStatusDraft({ ...statusDraft, monitoring_grace_minutes: Number(e.target.value) })} /></label>
