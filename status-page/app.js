(() => {
  ...
  function renderStates(sites) {
    ...
    $('sites').innerHTML = sites.map((site, index) => {
      ...
      const impact = site.status === 'outage' && site.impact_percent === 100 ? '100% of monitored service points affected' : site.status === 'degraded' && site.impact_percent != null ? `Approximately ${Number(site.impact_percent)}% of monitored service points affected` : site.status === 'degraded' ? 'Some service points are being assessed' : '';
