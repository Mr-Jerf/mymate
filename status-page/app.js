(() => {
  ...
  function renderStates(sites) {
    ...
    $('sites').innerHTML = sites.map((site, index) => {
      ...
      const impact = site.status === 'outage' && site.impact_percent === 100 ? '100% of devices are impacted' : site.status === 'degraded' && site.impact_percent != null ? `Approximately ${Number(site.impact_percent)}% of devices are impacted` : site.status === 'degraded' ? 'Some devices are being assessed' : '';
      return `<article class="state-card" id="site-${esc(site.key)}"><div class="state-head"><span class="state-name">${esc(label)}</span><span class="state-status ${site.status}">${stateLabel(site.status)}</span></div><div class="uptime">${site.uptime_60d == null ? '—' : `${Number(site.uptime_60d).toFixed(2)}%`}</div><div class="meta">${esc(meta)}</div>${impact ? `<div class="impact">${esc(impact)}</div>` : ''}<div class="history"><div class="history-title"><span>Last ${historyRange} days</span><span>Click a day for details</span></div><div class="bars${historyRange > 7 ? ' long-range' : ''}" style="--bar-count:${history.length}">${history.map((day) => { const hasIncident = (day.incidents || []).length > 0; const hasMaintenance = (day.maintenance || []).length > 0; const event = hasIncident || hasMaintenance; const barStatus = hasIncident && hasMaintenance ? 'stacked' : hasIncident ? 'outage' : hasMaintenance ? 'maintenance' : day.status; const statusLabel = hasIncident && hasMaintenance ? 'Outage and maintenance' : hasIncident ? 'Outage' : hasMaintenance ? 'Maintenance' : stateLabel(day.status); const segments = hasIncident && hasMaintenance ? '<span class="bar-segment outage"></span><span class="bar-segment maintenance"></span>' : ''; return `<a class="bar ${barStatus}" href="#activity" data-site="${esc(site.key)}" data-date="${esc(day.date)}" title="${esc(day.date)}: ${esc(statusLabel)}" aria-label="${esc(label)} ${esc(day.date)} ${esc(statusLabel)}">${segments}<span class="bar-tooltip" role="tooltip">${esc(tooltipDate(day.date))}</span></a>`; }).join('')}</div><div class="days${historyRange > 7 ? ' long-range-days' : ''}" style="--bar-count:${history.length}">${history.map((day) => `<span>${esc(barLabel(day.date))}</span>`).join('')}</div></div></article>`;
    }).join('');
  ...
  function historyIncidentHtml(incident) {
    ...
    const type = `<span class="history-type ${severity}">${severity === 'outage' ? 'Outage' : severity === 'degraded' ? 'Degraded' : 'Monitoring'}</span>`;
    const updates = (incident.updates || []).map((u) => `<p>${esc(u.message)} <span class="tag">${esc(fmt(u.created_at))}</span></p>`).join('');
    return `<details class="event ${cls}" open><summary><strong>${esc(incident.site || 'Site')} — ${esc(incident.summary)}</strong>${resolved}${type}<span class="tag">${esc(incidentStatusLabel(incident.status))} · ${esc(fmt(incident.started_at))}</span></summary>${updates || `<p>${count ? `${count} status update${count === 1 ? '' : 's'} recorded.` : 'No status updates recorded.'}</p>`}</details>`;
  }
